...
if(!class_exists('PDO')) {
	...
	class PDO {
		...
		function PDO($string_dsn, $string_username = '', $string_password = '', $array_driver_options = null) {
			...
			if($con['dbtype'] === 'mysql') {
				require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'PDO_mysql.class.php');
				if(isset($con['port']))
					$con['host'] .= ':'.$con['port'];
			...
			elseif($con['dbtype'] === 'sqlite2' || $con['dbtype'] === 'sqlite') {
				require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'PDO_sqlite.class.php');
				$this->__driver = new PDO_sqlite($con['dbname']);
			}
			elseif($con['dbtype'] === 'pgsql') {
				require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'PDO_pgsql.class.php');
