...
try {
    if (window.openDatabase) {
        db = openDatabase("xv-note", "1.0", "XVweb Backstage Note", 200000);
        if (!db)
...
Note.prototype = {
    ...
    save: function()
    {
        ...
        db.transaction(function (tx)
        {
            tx.executeSql("UPDATE WebKitStickyNotes SET note = ?, timestamp = ?, left = ?, top = ?, zindex = ? WHERE id = ?", [note.text, note.timestamp, note.left, note.top, /*note.zIndex*/ 4, note.id]);
        });
    },
