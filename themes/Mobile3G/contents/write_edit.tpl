...
<option value="4cs">GADV 4CS</option> 
	...
	<option value="z80">ZiLOG Z80 Assembler</option> 
		...
		<textarea class="EditArt xv-edit-textarea" id="EditArtPost" name="EditArtPost">{$ContextEdit}</textarea></td>
		<td width="25" style="vertical-align:top;">
		<a href="#" onclick="resizeelement('EditArtPost',40); return false;" title="{$language.Zoom}"><img src="{$UrlTheme}img/plus.gif" alt="{$language.Zoom}" /></a><br /><a href="#" onclick="resizeelement('EditArtPost',-40); return false;" title="{$language.ZoomOut}" ><img src="{$UrlTheme}img/minus.gif" alt="{$language.ZoomOut}" /></a>
		</td>
	</tr>
	<tr>
	...
	{if $WriteDescription}
		...
		{if "Amendments"|perm}<b><label for="amendmentID">{$language.Amendment}</label></b>: <input type="checkbox" id="amendmentID" name="amendment" class="StyleForm" value="true" /><br /><br />{/if}
		{/if}
...
<legend>{$language.LoadFromFile}</legend>
	...
	<form action="{$URLS.Script}DocToHtml/" method="post" enctype="multipart/form-data">
	<label for="doctohtml">.doc , .docx , .txt , .txd</label>
		<input name="doctohtml" type="file" id="doctohtml"> 
