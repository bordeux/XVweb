...
<!-- HTML5WYSIWYG Editor START --> 
		...
		<div class='xv-table'>
			...
			{if $WriteUrlArticle}
				<tr>
						<td><label for="xv-path"> {$language.WriteURL} :</label></td>
						<td> <input type="text" name="xv-path" class="StyleForm" value="{$smarty.get.url|escape:'html'|default:$UrlWrite}" style="width: 200px;" /></td>
				</tr>
			{/if}
			{if $WriteDescription}
				<tr>
						<td> <label for="Description">{$language.DescriptionOfChanges}  :</label></td>
						<td> <input type="text" name="xv-description" class="StyleForm" style="width: 200px;" /> </td>
				</tr>
				{if "Amendments"|perm}
					<tr>
							<td> <label for="amendment">{$language.Amendment}</label> :</td>
							<td> <input type="checkbox" name="amendment" class="xv-checkbox" value="true" /> </td>
					</tr>
				{/if}
			{/if}
				<tr>
						<td> <label for="arttitle">{$language.Title}</label> :</td>
						<td> <input type="text" name="arttitle" class="StyleForm" style="width: 200px;" value="{$TitleArt}" /> </td>
				</tr>
				<tr class="xv-comment-spambot">
						<td> <label for="xv-captcha"> {$language.RewriteKey} <span class="xv-comment-spambot-key">{$JSVars.SIDUser|substr:0:5}</span></label> :</td>
						<td> <input type="text" name="xv-captcha" value="" class="xv-comment-spambot-field" /> </td>
				</tr>	
				<tr>
						<td style="visibility:hidden;"><textarea style="display : none;" class="xv-textarea-wysiwyg" name="EditArtPost" ></textarea></td>
						<td> <input type="submit" name="save" value="{$language.Save}" /> </td>
				</tr>
			</table>
		</div>
		</form>
