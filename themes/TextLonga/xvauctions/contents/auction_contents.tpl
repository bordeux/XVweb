		...
		<div class="xvauction-info-left">
			...
			<table>
				...
				<tr>
					...
					<a href="{$URLS.Script}Users/{$auction_info.Seller|escape:'url'}/">{$auction_info.Seller}</a> (
						{$auction_seller_stats.percent}% /
						<span style="color:#02AB2C;">{$auction_seller_stats.positive}</span> /
						<span style="color:#949494;">{$auction_seller_stats.neutral}</span> /
						<span style="color:#CC0808;">{$auction_seller_stats.negative}</span>
					)
						<div>
							<a href="{$URLS.Script}Messages/{$auction_info.Seller|escape:'url'}/">{"xca_ask_question"|xv_lang}</a>
						</div>
						<div>
						...
						{elseif $auction_info.Type == "errand"}
							...
							{elseif $auction_info.Type == "dutch"}
								...
								<input type="hidden" name="auction" value="{$auction_info.ID}" />
									...
									<div>{"xca_cost"|xv_lang} : {$auction_info.BuyNow|number_format:2:'.':' '} {"xca_coin_type"|xv_lang}</div>
									<div>{"xca_pieces"|xv_lang} : <input type="number" min="1" max="{$auction_available_pieces}" name="pieces" style="width: 55px;" {literal}pattern="(([0-9]){0,})"{/literal} value="1" /></div>
									<input type="image" src="{$URLS.Theme}xvauctions/img/buy_now_big.png" style="width: 70px; height: 25px; background: none; border: none; " alt="{'xca_buynow'|xv_lang}">
								</form>
							{else}
								<form action="{$URLS.AuctionBuy}/{$auction_info.ID}/" method="post" style="text-align:center;">
								<input type="hidden" name="xv-sid" value="{$JSVars.SIDUser}" />
