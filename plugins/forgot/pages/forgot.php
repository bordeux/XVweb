<?php
xv_load_lang("forgot");
xv_trigger("forgot.forgot.start");
	...
	if($XVwebEngine->Session->get_sid() != ifsetor($_POST['xv_sid'], "")){
		...
		if($forgot_config->captcha_protection === false || ($forgot_config->captcha_protection === true && $_POST['xv_captcha'] == $XVwebEngine->Session->Session('Captcha_code')) ){
			...
			if(!empty($user_data)){
				...
				xvp()->user_send_email($users_class, $user_data->User , xv_lang("mail_reset_topic"), xv_lang("mail_reset_message"), array(
					"--xv-forgot-link--" => $URLS['Script'].'Forgot/Reset/'.$user_data->User.'/'.$hashed_password.'/'
				));
				
				$Smarty->assign('forgot_error', false);
				$Smarty->assign('forgot_success_mgs', xv_lang("forgot_success_message"));
