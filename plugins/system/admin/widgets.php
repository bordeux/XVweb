...
if(!isset($XVwebEngine)){
	...
	var $result = false;
		...
		$Widgets['administration']['widgets'][$_GET['wid']] = array(
			...
			if(isset( $_GET['xv-sid']) && $XVweb->Session->get_sid() == $_GET['xv-sid']){
				$this->result = $XVweb->user_config( $XVweb->Session->Session('user_name'), $Widgets);
			}else{
	...
	var $result = false;
		public function __construct(&$XVweb){
		$UserConfig = $XVweb->user_config($XVweb->Session->Session('user_name'));
			if(isset($UserConfig['administration']['widgets'][$_GET['wid']])){
				unset($UserConfig['administration']['widgets'][$_GET['wid']]);
				$this->result = $XVweb->user_config( $XVweb->Session->Session('user_name'), $UserConfig, false);
			}
