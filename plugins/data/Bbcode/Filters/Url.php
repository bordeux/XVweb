...
class BbCodeFilterUrl
{
	...
	public function findUrl($body, $parent, $settings)
	{
		...
		if(is_array($parent) && !BbCode::_checkAllowedTagName($parent, BbCode::CHECK_CHILD, 'url')) return false;
	...
	{
		if(isset($openNode['attributes']['tag_attributes']['url'])) return false;
		...
		foreach($body as &$el)
		{
			...
			if($el['type']==BbCode::NODE_TYPE_TEXT)
			{
				...
				if($str!==false)
				{
					if(!$inImg) $str = $this->shortUrl($el['text'], self::URL_LENGTH);
					break;
				}
			}
			...
			if($el['type']==BbCode::NODE_TYPE_CLOSE && $el['tagname'] == 'img') $inImg = false;
		}
