...
class BbCodeFilterImage
{
	...
	{	
		...
		$bodyStr='';
		foreach($body as $el) $bodyStr.=$el['text'];
			...
			))
		{
			if(ini_get('allow_url_fopen') && $this->checkRealImageSize)
			{
				...
				$socketTimeout=5;
				if(isset($this->socketTimeout) && $this->socketTimeout>=0) $socketTimeout=$this->socketTimeout;
				...
				)
			{
				...
				if($size[0]===false && $size[1]===false) $size=false;
			}
		...
		if($imgValue) $openNode['attributes']['tag_attributes']['img']=$imgValue;
		...
		foreach($body as $key=>&$el)
		{
			...
			else $el=array('type'=>0, 'text'=>'', 'tagText'=>$el['text']);
		}
