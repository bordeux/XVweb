...
include_once($GLOBALS['XVwebDir'].'libraries/formgenerator/validators.php');
...
$form->set("divs", true);                
$form->set("action", $GLOBALS['URLS']['AuctionPanel'].'/account_address/');
$form->set("method", "post");
$form->set("submitMessage", "Dane zostały zmienione");

$form->addField("text", "account_Corporation" , "Firma ", false , ifsetor($user_data['Corporation'], ''));
$form->validator("account_Corporation", "regExpValidator", "/^(.){0,50}$/", "Nazwa firmy za krótka");

$form->addField("text", "account_Name" , "Imię", true, ifsetor($user_data['Name'], '') );
$form->validator("account_Name", "regExpValidator", "/^(.){2,50}$/", "Imię za krótkie");

$form->addField("text", "account_Vorname" , "Nazwisko", true , ifsetor($user_data['Vorname'], ''));
$form->validator("account_Vorname", "regExpValidator", "/^(.){2,50}$/", "Nazwisko za krótkie");

$form->addField("text", "account_Street" , "Adres", true , ifsetor($user_data['Street'], ''));
$form->validator("account_Street", "regExpValidator", "/^(.){2,50}$/", "Zły adres");

$form->addField("text", "account_Zip" , "Kod pocztowy/ZIP code", true , ifsetor($user_data['Zip'], ''), ' placeholder="__-___"');
$form->validator("account_Zip", "regExpValidator", "/^((([0-9]){0,2}(\-)([0-9]){3}))$/", "Zły kod pocztowy/ZIP code");

$form->addField("text", "account_City" , "Miejscowość", true , ifsetor($user_data['City'], ''));
$form->validator("account_City", "regExpValidator", "/^(.){2,50}$/", "Miejscowość za krótka");

$form->addField("text", "account_State" , "Stan/Województwo", true , ifsetor($user_data['State'], ''));
$form->validator("account_State", "regExpValidator", "/^(.){2,50}$/", "Nazwa stanu/województwa za krótka");



$form->addField("select", "account_Country", "Kraj", true, ifsetor($user_data['Country'], ''), array_flip($_countries));
