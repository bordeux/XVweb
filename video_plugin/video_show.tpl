...
{/if}
	<video id="video_player" class="video-js vjs-default-skin" controls
	  preload="auto" poster="{$URLS.Site}video_plugin/jpg/{$file_info.ID}.jpg"
	  data-setup="{}">
	  <source src="{$URLS.Site}video_plugin/webm/{$file_info.ID}.webm" type='video/webm'>
	  <source src="{$URLS.Site}video_plugin/mp4/{$file_info.ID}.mp4" type='video/mp4'>
	  <source src="{$URLS.Site}video_plugin/ogg/{$file_info.ID}.ogg" type='video/ogg'>
	</video>
		<script>
